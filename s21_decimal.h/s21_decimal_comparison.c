...
int comparator(s21_decimal *x1, s21_decimal *x2) {
  ...
  int both_negative = 0;
  if (sign1 == 1 && sign2 == 0 &&
      (s21_is_zero_s21_decimal(*x1) || s21_is_zero_s21_decimal(*x2)))
    res = -1;
  ...
  if (!res) {
    ...
    s21_init_big(*x1, &big1);
    s21_init_big(*x2, &big2);
    ...
    if (difference_position > 0) {
      ...
      for (int i = big1.one_position_left; i >= 0; i--) {
        difference_bit = s21_get_bit_big(&big1, i) - s21_get_bit_big(&big2, i);
        if (difference_bit > 0) {
          res = 1;
          break;
